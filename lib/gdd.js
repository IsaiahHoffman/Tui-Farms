...
// ---------- variety thresholds ----------
// Varieties are parameterized around PRIME — the stage the farm judges most
// confidently in the field:
//   primeGDD        absolute GDD from planting to prime eating quality
//   earlyMinusGDD   how many GDD BEFORE prime the first pickable ears show
//   freezerPlusGDD  how many GDD AFTER prime the block turns freezer-best
//   donePlusGDD     how many GDD after prime the block is finished
//                   (null = unknown; treated as freezer-best + 80)
// This helper is the single place those become the absolute per-stage
// thresholds the projection walks over.
function deriveThresholds(variety) {
  const prime = variety.primeGDD;
  const early = prime - variety.earlyMinusGDD;
  const mature = prime + variety.freezerPlusGDD;
  const done = Number.isFinite(variety.donePlusGDD)
    ? prime + variety.donePlusGDD
    : mature + 80;
  return { early, prime, mature, done };
}

...
// A PROJECTED end past the frost cutoff is capped to the frost phrase, same
// as the cards above it — but an observed/actual date is history, never capped.
function buildCalendar(blocks, frostWatch) {
  ...
  return Array.from(byVariety.values())
    ...
    .map(e => {
      // approx === true means the date is projected (forecast / seasonal
      // estimate); approx === false means it actually happened.
      const endLabel = (frostWatch && e.last.approx && e.last.date > frostWatch)
        ? `~${frostPhrase(frostWatch)}, weather permitting`
        : maybeTilde(e.last.date, e.last.approx);
      return {
        variety: e.variety,
        range: `first pick ${maybeTilde(e.first.date, e.first.approx)} to ${endLabel}`,
      };
    });
}
...
function buildCornCast(varieties, plantings, weatherByDate, today, frostWatch) {
  ...
  return {
    ...
    calendar: buildCalendar(blocks, frostWatch),
  };
}
